-- Insert base languages for popular language learning
-- Note: These will only be inserted if they don't already exist for this user
-- You'll need to run this for each user or modify to create as templates

-- For now, let's create a function that users can call to populate their account with starter data
CREATE OR REPLACE FUNCTION public.create_starter_language_data(p_user_id UUID, p_language_name TEXT, p_flag_emoji TEXT)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- Insert language
  INSERT INTO public.languages (user_id, name, flag_emoji)
  VALUES (p_user_id, p_language_name, p_flag_emoji)
